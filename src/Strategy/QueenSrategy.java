...
public class QueenSrategy implements MoveStrategy{
    private int curX;
    private int curY;

    public QueenSrategy(int curX, int curY) {
        this.curX = curX;
        this.curY = curY;
    }
    ...
    public boolean move (int x, int y, Set<ChessPiece> chessPieces) {
        if(Math.abs(x-curX)==Math.abs(y-curY) || !(x!=curX && y!=curY)){
            if(isOverPiece(Math.min(curX,x) , Math.min(curY,y) , Math.max(curX,x) , Math.max(curY,y) , chessPieces))
                return false;
            curX = x;
            curY = y;
            return true;
